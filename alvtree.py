...
class Avltree:
    """
    AVL木（Adelson-Velskii and Landis' tree; 平衡二分木）
    昇順に挿入、近しい値のindexの取得などをO(log n)で行ってくれるデータ構造
    pypy3でないと、O(2*10^5)でTLEとなるため注意
...
from itertools import permutations #PyPy3
# -> for x in permutations("abc"): print(x)で、('a', 'b', 'c'), ..., ('c', 'b', 'a')と辞書順に並べてくれる
# -> input()で得たものをやるときは、... permutations(str[0:len(str)-1])とやらないと、"\n"が入る。
from itertools import combinations #PyPy3
# -> for pair in combinations(li, m):
# -> liからm個ずつ取ってきて、ペアのタプルを作成してくれる
import collections #PyPy3
import statistics as st #PyPy3
import heapq as hq #PyPy3
import re #Pypy3
import math #PyPy3

from scipy.special import comb, perm
from scipy import optimize
from bisect import bisect, bisect_left #二分探索
import numpy as np
...
for _ in range(q):
    ...
    if c == 1:
        ...
        print(avl.search_higher(x, l) - avl.search_lower(x, 0))
